...
library and Nussknacker service. For this purpose a separate directory named dev-environment was
...
The process of creating the new environment comes to running a single bash script from the dev-environment
...
environment) or to exclude the process of library recompilation before placing it in the Nusskcacer image.
...
Every integration has its own docker-compose configuration file which specifies the Docker services
...
external services for the other integrations and Nusskacker environment but create a single proxy
...
The Docker images for each integration needs extra dependencies which are managed using the conda
...
downloading the image. However, this GitHub politics can change in the near future as the community
...
environment configuration is loaded with the sbt plugin from the .env file. However, in the case of need
to run tests from an IDE we need to specify separately the .env file in test run configuration as for now
